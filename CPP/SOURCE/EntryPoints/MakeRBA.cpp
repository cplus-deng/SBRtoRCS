...
int main( int argc, char *argv[] )
{
	/*
		Args (3 total):
		[0]: executable name
		[1]: input .unv file name
		[2]: output .rba file name
	*/

	std::cout << "## MakeRBA ##" << std::endl;

	auto tStart = Clock::now();

	if( argc != 3 )
	{
		std::cerr << "Command line arg count must be 2!" << std::endl;
		exit( 1 );
	}
	else
	{
		...
		std::string stlFilePath = std::string("E:\\Projects\\RaytrAMP-master\\CPP\\Test\\sphere_r05m.stl");
		...
		reducedBvhArray.Save( rbaFilePath );
