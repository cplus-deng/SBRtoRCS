...
namespace LUV
{
	...
	// SCALAR * VECTOR Changed by dy 2023/12/2, return type T -> S
	template< unsigned int N, class T, class S >
	inline LuVector< N, S > operator *( const T& lhs, const LuVector< N, S >& rhs ) restrict( cpu, amp )
	{
		LuVector< N, S > result( lhs );
		return result *= rhs;
	}
