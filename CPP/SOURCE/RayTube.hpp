...
template< class T >
class RayTubeCorner :public RayTube < T >
{
public:
	std::shared_ptr<LUV::Vec3< T >>refCoords_;
	std::shared_ptr<U32>hitIdx_;
};
...
public:
	LUV::Vec3< T > pos_;
	LUV::Vec3< T > dir_;
	LUV::Vec3< std::complex< T > > pol_;
	LUV::Vec3< T > refNormal_;
	T dist_;
	T delay_;
	U32 refCount_;
	U32 lastHitIdx_;
	std::shared_ptr<std::complex< T >> es_;//remove 
	U32 nDelay_[N_Ref_Cnt];
	std::complex< T >eqPol_[N_Ref_Cnt];

public:
	//
	RayTubeDie() //restrict( cpu, amp ) 
	{
	...
	RayTubeDie(
		const LUV::Vec3< T >& pos,
		const LUV::Vec3< T >& dir,
		const LUV::Vec3< std::complex< T > >& pol,
		const T& dist,
		const LUV::Vec3< T >& refNormal,
		const U32& refCount,
		const U32& lastHitIdx
...
public:
	...
	U32 refCount_;
	U32 lastHitIdx_;
	LUV::Vec3< T > refCoords_[N_Corner];
	U32 hitIdx_[N_Corner];
	/*std::shared_ptr<LUV::Vec3< T >>refCoords_;
	std::shared_ptr<U32>hitIdx_;*/

public:
	//
	RayTubeCorDie() //restrict( cpu, amp ) 
	{
