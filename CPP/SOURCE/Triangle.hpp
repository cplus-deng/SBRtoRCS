...
public:
	...
	Triangle(const LUV::Vec3< T >& v1, const LUV::Vec3< T >& v2, const LUV::Vec3< T >& v3, 
		const LUV::Vec3< T >& normal,const U32 mtype)restrict(cpu, amp):
		v1_(v1),
		v2_(v2),
		v3_(v3),
		normal_(normal),
	...
	LUV::Vec3< T > GetLoadNormal() const restrict(cpu, amp)
	{
		return normal_;
	}
